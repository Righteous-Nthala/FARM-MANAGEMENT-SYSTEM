...
class _FertilizerRecordsPageState extends State<FertilizerRecordsPage> {
  final CollectionReference fertilizerRecordsCollection =
  FirebaseFirestore.instance.collection('fertilizer_records');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder(
        stream: fertilizerRecordsCollection.snapshots(),
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return Center(
              child: Column(
                ...
                children: [
                  const Text("No fertilizer records found"),
                  const SizedBox(height: 20),
                  ElevatedButton(
                    onPressed: _showAddFertilizerRecordDialog,
                    child: const Text("Add Fertilizer Record"),
                  ),
                ],
              ),
            );
            ...
            return SingleChildScrollView(  // Make the table scrollable vertically
              child: SingleChildScrollView(  // Allow horizontal scrolling
                ...
                child: DataTable(
                  ...
                  columns: const [
                    DataColumn(label: Center(child: Text("Row No."))),
                    DataColumn(label: Center(child: Text("Type"))),
                    DataColumn(label: Center(child: Text("Quantity (kg)"))),
                  ...
                  rows: List.generate(snapshot.data!.docs.length, (index) {
                    ...
                    final String rowNumber = (index + 1).toString();
                        ...
                        DataCell(Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () => _deleteFertilizerRecord(doc.id),
                            ),
                          ],
                        )),
                      ],
  ...
  Future<void> _showAddFertilizerRecordDialog({
    String action = 'Add',
    String? docId,
    String type = '',
    String quantity = '',
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text("$action Fertilizer Record"),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final duplicateExists = await _checkForDuplicateFertilizerRecord(
              ...
              if (action == 'Add') {
                await _addFertilizerRecord(type, quantity, applicationDate, crop);
              } else {
                await _editFertilizerRecord(docId!, type, quantity, applicationDate, crop);
              }
            },
  ...
  Future<void> _addFertilizerRecord(String type, String quantity,
      String applicationDate, String crop) async {
    await fertilizerRecordsCollection.add({
      'type': type,
      'quantity': quantity,
  ...
  Future<void> _editFertilizerRecord(String docId, String type, String quantity,
      String applicationDate, String crop) async {
    await fertilizerRecordsCollection.doc(docId).update({
      'type': type,
      'quantity': quantity,
      'application_date': applicationDate,
      'crop': crop,
  ...
  Future<void> _deleteFertilizerRecord(String docId) async {
    await fertilizerRecordsCollection.doc(docId).delete();
  }

  Future<bool> _checkForDuplicateFertilizerRecord(
      String type, String quantity, String applicationDate,
      String crop, String? docId) async {
    final querySnapshot = await fertilizerRecordsCollection
